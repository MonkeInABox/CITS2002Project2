#include "mysync.h"

//copies the up to date file into the out of date file.
void copyfile(struct stat *stats1, int index1, struct stat stats2, char *dirName2, char *fileName2){
    //checks which file is out of date
    if(&stats1->st_mtime > &stats2.st_mtime){
        //paths of the files are created and intitialised to the path arrays
        char path1[512];
        char path2[512];
        printf("%s\n",strcat(strcat(realpath(dirNames[index1], path1),"/"), newDir[index1]));
        printf("%s\n",strcat(strcat(realpath(dirName2, path2),"/"), fileName2));
        //files are opened
        FILE *sourceFile  = fopen(path1, "r");
        FILE *destFile    = fopen(path2, "w"); 
        //checks if opening of files succeed 
        if (sourceFile == NULL || destFile == NULL)
        {
            printf("\nUnable to open file.\n");
            exit(EXIT_FAILURE);
        }
        //copies all contents from one file to the other
        char ch = fgetc(sourceFile);
        while (ch != EOF)
        {
            fputc(ch, destFile);
            ch = fgetc(sourceFile);
        }
        //files are closed
        fclose(sourceFile);
        fclose(destFile);
    }
    //checks which file is out of date
    if(&stats1->st_mtime < &stats2.st_mtime){
        char path2[512];
        char path1[512]; 
        printf("%s\n",strcat(strcat(realpath(dirName2, path1),"/"), fileName2));
        printf("%s\n",strcat(strcat(realpath(dirNames[index1], path2),"/"), newDir[index1]));
        FILE *sourceFile  = fopen(path2, "r");
        FILE *destFile    = fopen(path1, "w");  
        if (sourceFile == NULL || destFile == NULL)
        {
            printf("\nUnable to open file.\n");
            exit(EXIT_FAILURE);
        }
        char ch = fgetc(sourceFile);
        while (ch != EOF)
        {
            fputc(ch, destFile);
            ch = fgetc(sourceFile);
        }
        fclose(sourceFile);
        fclose(destFile);
    }
}